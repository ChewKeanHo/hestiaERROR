...
function hestiaTIME-Format-Date-RPM {
        param (
        ...
        return $___t.ToString("ddd MMM dd yyyy")
}
################################################################################
# Windows POWERSHELL Codes                                                     #
################################################################################
...
hestiaTIME_Format_Date_RPM() {
        ...
        if [ "$(echo "$(uname)" | tr '[:upper:]' '[:lower:]')" = "darwin" ]; then
                printf -- "%b" "$(date -j -f "%s" "${1}" +"%a %b %d %Y")"
        else
                printf -- "%b" "$(date --date="@${1}" +"%a %b %d %Y")"
        fi
