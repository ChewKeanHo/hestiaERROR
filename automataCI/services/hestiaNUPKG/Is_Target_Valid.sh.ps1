...
. "${env:LIBS_HESTIA}\hestiaZIP\Vanilla.sh.ps1"




function hestiaNUPKG-Is-Target-Valid {
        param (
                [string]$___target
        )
        ...
        if ($(hestiaZIP-Is-Target-Valid "${___target}") -ne ${env:hestiaKERNEL_ERROR_OK}) {
                return ${env:hestiaKERNEL_ERROR_DATA_MISSING}
        }
...
. "${LIBS_HESTIA}/hestiaZIP/Vanilla.sh.ps1"




hestiaNUPKG_Is_Target_Valid() {
        #___target="$1"
        ...
        if [ $(hestiaZIP_Is_Target_Valid "$1") -ne $hestiaKERNEL_ERROR_OK ]; then
        ...
        if [ ! "${___target%%.nupkg*}" = "$___target" ]; then
