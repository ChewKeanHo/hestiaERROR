...
function hestiaNUPKG-Is-Activated {
        # execute
        if ($(hestiaSTRING-Is-Empty ${env:DOTNET_ROOT}) -eq ${env:hestiaKERNEL_ERROR_OK}) {
                return ${env:hestiaKERNEL_ERROR_NOT_POSSIBLE}
        }

        if ($(hestiaOS-Is-Command-Available "dotnet") -ne ${env:hestiaKERNEL_ERROR_OK}) {
                return ${env:hestiaKERNEL_ERROR_NOT_POSSIBLE}
        }
...
hestiaNUPKG_Is_Activated() {
        # execute
        if [ $(hestiaSTRING_Is_Empty "$DOTNET_ROOT") -eq $hestiaKERNEL_ERROR_OK ]; then
                printf -- "%d" $hestiaKERNEL_ERROR_NOT_POSSIBLE
                return $hestiaKERNEL_ERROR_NOT_POSSIBLE
        fi

        if [ $(hestiaFS_Is_File "${DOTNET_ROOT}/dotnet") -eq $hestiaKERNEL_ERROR_OK ]; then
                printf -- "%d" $hestiaKERNEL_ERROR_NOT_POSSIBLE
                return $hestiaKERNEL_ERROR_NOT_POSSIBLE
        fi
