...
function hestiaGZ-Is-Target-Valid {
        param (
                [string]$___target
        )
        ...
        # validate input
        if ($(hestiaSTRING-Is-Empty $___target) -eq $hestiaKERNEL_ERROR_OK) {
        ...
        if ($($___target -replace '.*\.tgz$', '') -ne $___target) {
                return ${env:hestiaKERNEL_ERROR_OK}
        }

        if ($($___target -replace '.*\.gz$', '') -ne $___target) {
                return ${env:hestiaKERNEL_ERROR_OK}
        }
...
hestiaGZ_Is_Target_Valid() {
        ...
        if [ ! "${___target%%.tgz*}" = "$___target" ]; then
                printf -- "%d" $hestiaKERNEL_ERROR_OK
                return $hestiaKERNEL_ERROR_OK
        fi

        if [ ! "${___target%%.gz*}" = "$___target" ]; then
                printf -- "%d" $hestiaKERNEL_ERROR_OK
                return $hestiaKERNEL_ERROR_OK
        fi
