...
. "${env:LIBS_HESTIA}\hestiaTAR\Is_Target_Valid.sh.ps1"




function hestiaNPM-Is-Target-Valid {
        param (
                [string]$___target
        )
        ...
        if ($(hestiaTAR-Is-Target-Valid $___target) -ne ${env:hestiaKERNEL_ERROR_OK}) {
...
. "${LIBS_HESTIA}/hestiaTAR/Is_Target_Valid.sh.ps1"




hestiaNPM_Is_Target_Valid() {
        #___target="$1"
        ...
        if [ $(hestiaTAR_Is_Target_Valid "$1") -ne $hestiaKERNEL_ERROR_OK ]; then
        ...
        if [ ! "${___target%%_js-js.tgz*}" = "$___target" ]; then
