...
function hestiaCONSOLE-Log-Check {
        param (
                [string]$___subject,
                [string]$___lang
        )
        ...
        $null = hestiaCONSOLE-Log info `
                "$(hestiaI18N-Translate-Check "${___lang}" "${___subject}")`n" `
                "${___lang}"
...
hestiaCONSOLE_Log_Check() {
        ...
        hestiaCONSOLE_Log info "$(hestiaI18N_Translate_Check "$___lang" "$1")\n" "$___lang"
