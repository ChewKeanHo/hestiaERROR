...
function hestiaCONSOLE-Log-Recreate-Failed {
        param (
                [string]$___appendix,
                [string]$___lang
        )
        ...
        $null = hestiaCONSOLE-Log error `
                "$(hestiaI18N-Translate-Recreate-Failed $___lang)`n" `
                $___lang `
                $___appendix
...
hestiaCONSOLE_Log_Recreate_Failed() {
        #___appendix="$1"
        #___lang="$2"
        ...
        hestiaCONSOLE_Log error \
                "$(hestiaI18N_Translate_Recreate_Failed "$___lang")\n" \
                "$___lang" \
                "$1"
