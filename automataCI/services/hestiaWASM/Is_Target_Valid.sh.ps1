...
function hestiaWASM-Is-Target-Valid {
        param (
                [string]$___target
        )
        ...
        if (
                ($("${___target}" -replace '.*-wasm.*', '') -ne "${___target}") -or
                ($("${___target}" -replace '.*\.wasm$', '') -ne "${___target}")
        ) {
...
hestiaWASM_Is_Target_Valid() {
        ...
        if [ ! "${___target##*-wasm}" = "$___target" ] ||
                [ ! "${___target%%.wasm*}" = "$___target" ]; then
