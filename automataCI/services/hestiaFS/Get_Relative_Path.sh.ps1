...
function hestiaFS-Get-Relative-Path {
        param (
                [string]$___source,
                [string]$___base
        )
        ...
        # validate input
        if (
                ($(hestiaSTRING-Is-Empty $___filepath) -eq ${env:hestiaKERNEL_ERROR_OK}) -or
                ($(hestiaSTRING-Is-Empty $___base) -eq ${env:hestiaKERNEL_ERROR_OK})
        ) {
                return ""
        }
