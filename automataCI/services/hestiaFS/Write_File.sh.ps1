...
function hestiaFS-Write-File {
        ...
        if ($(hestiaSTRING-Is-Empty $___string_content) -eq ${env:hestiaKERNEL_ERROR_OK}) {
                ...
                return hestiaFS-Write-Byte-File $___target $___byte_content
        } else {
                # default to text data type
                return hestiaFS-Write-Text-File $___target $___string_content
        }
}
################################################################################
# Windows POWERSHELL Codes                                                     #
...
hestiaFS_Write_File() {
        ...
        if [ $(hestiaSTRING_Is_Empty "$2") -eq $hestiaKERNEL_ERROR_OK ]; then
                # write as byte data type
                hestiaFS_Write_Byte_File "$1" "$2"
        else
                # default to text data type
                hestiaFS_Write_Text_File "$1" "$2"
        fi
