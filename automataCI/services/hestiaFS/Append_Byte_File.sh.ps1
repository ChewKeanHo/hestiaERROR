...
function hestiaFS-Append-Byte_File {
        param (
                [string]$___target,
                [byte[]]$___content
        )
        ...
        if (
                ($(hestiaFS-Is-Exist $___target) -eq ${env:hestiaKERNEL_ERROR_OK}) -and
                ($(hestiaFS-Is-File $___target) -ne ${env:hestiaKERNEL_ERROR_OK})
        ) {
        ...
        foreach ($___byte in $___content) {
                $null = Add-Content -AsByteStream -NoNewline -Path $___target -Value $___byte
                if (-not $?) {
...
hestiaFS_Append_Byte_File() {
        ...
        if [ $(hestiaFS_Is_Exist "$1") -eq $hestiaKERNEL_ERROR_OK ] &&
                [ $(hestiaFS_Is_File "$1") -ne $hestiaKERNEL_ERROR_OK ]; then
