...
. "${env:LIBS_HESTIA}\hestiaCHOCOLATEY\Is_Available.sh.ps1"
...
function hestiaCHOCOLATEY-Install {
        param (
                [string]$___package
        )
        ...
        # execute
        $___process = OS-Exec "choco" "install `"${___package}`""
...
. "${LIBS_HESTIA}/hestiaCHOCOLATEY/Is_Available.sh.ps1"
. "${LIBS_HESTIA}/hestiaSTRING/Is_Empty.sh.ps1"




hestiaCHOCOLATEY_Install() {
        #___package="$1"


        # validate input
        if [ $(hestiaCHOCOLATEY_Is_Available) -ne $hestiaKERNEL_ERROR_OK ]; then
